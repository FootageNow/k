...
async def create_mission(interaction: discord.Interaction, roblox_name: str, skin_url: str):
    uid = str(interaction.user.id)
    team_name = data["teams"].get(uid)
    if not team_name:
        return await interaction.response.send_message("You do not own a team.", ephemeral=True)
    guild = interaction.guild
    category = None
    # find missions🛡 category
    for cat in guild.categories:
        if cat.name == "missions🛡":
            category = cat
            break
    if not category:
        return await interaction.response.send_message("No missions category found.", ephemeral=True)
    embed = discord.Embed(title=f"New Mission for {roblox_name}", description=f"Player: {roblox_name}")
    embed.set_image(url=skin_url)
    # send to all text channels in category
    for ch in category.text_channels:
        await ch.send(embed=embed)
    await interaction.response.send_message("Mission created and sent to missions channels.", ephemeral=True)
